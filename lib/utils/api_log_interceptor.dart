...
class ApiLogInterceptor extends Interceptor {
  ...
  static String tagFor(RequestOptions options) {
    ...
    if (path.contains('/screens/screenshot')) return 'ScreenAPI';
  ...
  void onResponse(
    ...
    if (logResponseBody) {
      AppLogger.api(
        tag,
        '  response: ${_formatBody(response.data, response.requestOptions)}',
      );
    }
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    if (err.response?.data != null) {
      AppLogger.api(
        tag,
        '  error body: ${_formatBody(err.response!.data, err.requestOptions)}',
      );
    }
    handler.next(err);
  }
  ...
  String _formatBody(dynamic data, RequestOptions options) {
    ...
    if (data is String) {
      if (options.uri.toString().contains('xmds.php') &&
          data.contains('<file>')) {
        return '<SOAP GetFile ${data.length} chars — base64 omitted>';
      }
      if (data.length > 2000 &&
          options.uri.toString().contains('method=GetFile')) {
